"""
feature_asymmetry.py — Asymmetry feature extraction
=====================================================
Evaluates lesion shape asymmetry by XORing mirrored mask halves
at multiple orientations and averaging the scores.

Score of 0 = perfectly symmetric, 1 = fully asymmetric.
Returns a dataframe — does not save to disk (main.py builds the master CSV).

Output columns:
    image_name      — filename
    asymmetry_score — mean score across all valid rotations (0-1)
    rotations_used  — number of rotations that produced a valid score
"""
...
def feature_asymmetry(mask, n=N_ROTATIONS):
    """
    Rotate mask n times between 0 and 90 degrees and average scores.
    Averaging across orientations makes the result rotation-invariant.
    """
    scores = []
    for i in range(n):
        degrees   = 90 * i / n
